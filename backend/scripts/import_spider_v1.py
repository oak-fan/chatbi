"""导入 Spider v1 (dev + test) 到 ChatBI benchmark 业务表。

用法：
  python scripts/import_spider_v1.py --root /path/to/Spider --user-id 1
  python scripts/import_spider_v1.py --split dev --skip-preprocess
  python scripts/import_spider_v1.py --split test --skip-preprocess
  python scripts/import_spider_v1.py --split both --skip-preprocess
"""
...
SplitType = Literal["dev", "test", "both"]
...
def _load_samples(root: Path, split: str) -> list[dict[str, Any]]:
    filename = "dev.json" if split == "dev" else "test.json"
    path = root / filename
    if not path.is_file():
        raise FileNotFoundError(f"Spider {split} 样本文件不存在: {path}")
    return json.loads(path.read_text(encoding="utf-8"))


def _db_file(root: Path, db_id: str, split: str) -> str | None:
    """返回相对于 CHATBI_BENCHMARK_ROOT 的 SQLite 路径；数据库不存在时返回 None。"""
    settings = _settings()
    benchmark_root = Path(settings.chatbi_benchmark_root).resolve()
    spider_rel = root.resolve().relative_to(benchmark_root)
    dir_name = "test_database" if split == "test" else "database"
    relative = spider_rel / dir_name / db_id / f"{db_id}.sqlite"
    if (benchmark_root / relative).is_file():
        return relative.as_posix()
    fallback_dir = "database" if split == "test" else "test_database"
    relative2 = spider_rel / fallback_dir / db_id / f"{db_id}.sqlite"
    if (benchmark_root / relative2).is_file():
        return relative2.as_posix()
    return None
...
async def _ensure_datasource(
    *,
    ...
    payload = ChatbiDatasourceCreateInput(
        ...
        remark=f"Spider v1 {dataset_prefix} SQLite benchmark datasource",
    )
...
async def import_spider_split(
    *,
    ...
    samples = _load_samples(root, split)
    ...
    dataset_code = "SPIDER"
    dataset_version = split.upper()
    display_name = f"Spider {split.upper()}"
    dataset_prefix = f"SPIDER-{dataset_version}"
    remark = f"Spider v1 {split} SQLite benchmark dataset"
    ...
    try:
        async with database.get_session() as session:
            ...
            for db_id in db_ids:
                db_file = _db_file(root, db_id, split)
                if db_file is None:
                    skipped_dbs.append(db_id)
                else:
                    available_dbs[db_id] = db_file
            if skipped_dbs:
                print(f"  跳过 {len(skipped_dbs)} 个无 SQLite 数据库的 db_id: {', '.join(skipped_dbs)}")
            datasource_by_db: dict[str, int] = {}
            for order, db_id in enumerate(sorted(available_dbs)):
                datasource_id = await _ensure_datasource(
                    ds_repo=ds_repo,
                    user_id=user_id,
                    db_id=db_id,
                    db_file=available_dbs[db_id],
            ...
            for index, item in enumerate(samples):
                ...
                if db_id not in datasource_by_db:
                    continue
                gold_sql = str(item.get("query") or "")
                evidence = str(item.get("evidence") or "")
                question_id = item.get("question_id", index)
                await benchmark_repo.upsert_sample(
                    sample_code=f"{db_id}:{question_id}:{index}",
                    dataset_id=dataset_id,
            ...
            await unit_of_work.commit()
            print(f"Imported Spider {dataset_version}: dataset_id={dataset_id}, samples={imported}/{len(samples)}, db_ids={len(datasource_by_db)}/{len(db_ids)}")
            if skipped_dbs:
                print(f"  跳过的 db_id（无 SQLite）: {', '.join(skipped_dbs)}")
    finally:
...
def _resolve_root(raw_root: str | None) -> Path:
    ...
    if settings.chatbi_benchmark_root is not None:
        return (Path(settings.chatbi_benchmark_root) / "Spider").resolve()
    return Path("/mnt/benchmarks/Spider").resolve()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="导入 Spider v1 benchmark 数据")
    parser.add_argument(
        "--root",
        default=None,
        help="Spider 根目录（含 dev.json / test.json 与 database/）",
    )
